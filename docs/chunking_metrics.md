...
Perplexity измеряет неопределённость языковой модели при предсказании последовательности токенов. Для последовательности $x_{1:n}$ она определяется как экспонента среднего negative log-likelihood:

$$
PPL(x_{1:n}) = \exp\left(-\frac{1}{n}\sum_{t=1}^{n}\log p(x_t\mid x_{<t})\right),
$$

где $p(x_t\mid x_{<t})$ — вероятность следующего токена при известных предыдущих токенах. Чем ниже perplexity, тем увереннее модель предсказывает текст; интуитивно значение можно понимать как эффективное среднее число равновероятных вариантов следующего токена.

В Boundary Clarity контекст $d$ используется как дополнительное условие. Если $PPL(q\mid d)$ заметно ниже $PPL(q)$, предыдущий чанк существенно уменьшает неопределённость при предсказании следующего, а значит, между чанками сохраняется сильная зависимость. Абсолютные значения perplexity нельзя напрямую сравнивать между разными моделями и токенизаторами.
